...
app.layout = html.Div(
    children=[
        html.H1(children="Average order quantity"),
        html.P(
            children=(
...
def update_charts(vendors,start_date,end_date):
    ...
    average_orders_figure = {
        "data": [
            {
                "x": filtered_data["Date"],
                "y": filtered_data["average ordered"],
                "type": "bar",
                "hovertemplate": '<br>'.join([ "avg:%{y:.2f}<extra></extra>",'Date:%{x}'])
            },
        ]
    }
    return(average_orders_figure)
